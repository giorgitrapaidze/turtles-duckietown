  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::duckietown_msgs::LEDPattern_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.color_list.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.color_list[i]);
    }
    if (v.color_list.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.rgb_vals.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::std_msgs::ColorRGBA_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.rgb_vals[i]);
    }
    if (v.rgb_vals.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.color_mask.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.color_mask[i]);
    }
    if (v.color_mask.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "frequency_mask: ";
    if (v.frequency_mask.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.frequency_mask.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<int8_t>::stream(s, true ? std::string() : indent + "    ", v.frequency_mask[i]);
    }
    if (v.frequency_mask.empty() || true)
      s << "]";
  }
