  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::duckietown_msgs::AprilTagsWithInfos_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.detections.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::duckietown_msgs::AprilTagDetection_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.detections[i]);
    }
    if (v.detections.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.infos.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::duckietown_msgs::TagInfo_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.infos[i]);
    }
    if (v.infos.empty() || false)
      s << "]";
  }
